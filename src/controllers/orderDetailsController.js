        ...
        for (const detail of orderDetails) {
            ...
            if (product) {
                ...
                if (detail.discount_id) {
                    // Если есть скидка, применяем её к цене товара
                    const discount = await Discount.findByPk(detail.discount_id);
                    if (discount) {
                        price *= (1 - discount.percent / 100); // Применяем скидку к цене
                    }
                }
                totalAmount += detail.quantity * price; // Умножаем на количество товара
            }
        }
